  ...
  return (
    <div className="space-y-8">
      <form method="get" action="/produits" className="space-y-5">
        ...
        <div>
          <label htmlFor="tri" className="mb-1.5 block text-sm font-medium text-ink">
            Trier par
          </label>
          <select
            id="tri"
      ...
      <div>
        <h3 className="mb-3 text-sm font-medium text-ink">Catégories</h3>
        <ul className="space-y-1">
          <li>
            <Link
              href={buildHref('/produits', searchParams, { categorie: undefined, page: undefined })}
              className={cn(
                'block rounded px-2 py-1.5 text-sm transition-colors hover:bg-paper-muted',
                !currentCategory ? 'font-medium text-accent' : 'text-ink-muted'
              )}
            >
          ...
          {categories.map((category) => (
            <li key={category.id}>
              <Link
                ...
                className={cn(
                  'block rounded px-2 py-1.5 text-sm transition-colors hover:bg-paper-muted',
                  currentCategory === category.slug ? 'font-medium text-accent' : 'text-ink-muted'
                )}
              >
                {category.name}
              </Link>
